...
async function carregarProdutos() {
    try {
        ...
        console.log('Resposta da API /api/produtos:', response);
        if (response.ok) {
...
function exibirProdutos() {
    ...
    tbody.innerHTML = produtos.map((produto, idx) => {
        ...
        const margem = produto.margem ? parseFloat(produto.margem) : ((preco > 0 && custo > 0) ? ((preco - custo) / custo * 100) : 0);
        const lucroUnitario = preco - custo;
        const lucroTotal = lucroUnitario * (produto.estoque || 0);
        return `
        <tr>
            ...
            <td>
                <span class="badge-lucro">
                    ${margem.toFixed(2)}% / R$ ${lucroUnitario.toFixed(2)}
                </span>
            </td>
...
async function salvarProduto(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Recarregar produtos direto da API
            await carregarProdutos();
        } else {
            const erro = await response.json();
            alert('Erro ao salvar produto: ' + (erro.mensagem || 'Erro desconhecido'));
        }
    } catch (error) {
...
function deletarProduto(idx) {
    if (confirm('Deseja realmente excluir este produto?')) {
        const produto = produtos[idx];
        console.log('Deletando produto ID:', produto.id);
